...
import { CHAIN_LIST_USDC } from '@/constants';
...
export default function TokenSelect(props: any) {
  ...
  // Map wagmi chainId to CHAIN_LIST_USDC index
  ...
  const availableChains = isFirst 
    ? CHAIN_LIST_USDC 
    : CHAIN_LIST_USDC.filter((_, index) => index !== opChain);
    ...
    <div className="relative w-full h-12" ref={newRef}>
      ...
      >
        <div className="text-lg">
          {chain == null ? "Select Chain" : CHAIN_LIST_USDC[chain].chain}
        </div>
        <span
      ...
      {showMenu && (
        <div className="absolute w-full top-[110%] left-0 max-h-[300px] bg-primary-[#BD4822] shadow shadow-[#000] backdrop-blur rounded-lg p-2 z-50 overflow-auto">
          {availableChains.map((item, index) => {
            const originalIndex = CHAIN_LIST_USDC.findIndex(c => c.id === item.id);
            return (
              <div 
                key={originalIndex} 
